...
// Vercel/container filesystems are ephemeral; /tmp is the safe writable area.
const ROOT = process.env.WORK_DIR || path.join('/tmp', 'linguadub-data');
const UPLOADS = path.join(ROOT, 'uploads');
const DIST = path.resolve(process.cwd(), 'dist');

const langCodes = {
  Gujarati: 'gu', Hindi: 'hi', Marathi: 'mr', English: 'en', Bengali: 'bn',
...
const voices = {
  ...
  Telugu: 'te-IN-ShrutiNeural', Kannada: 'kn-IN-SapnaNeural', Malayalam: 'ml-IN-SobhanaNeural',
...
function run(cmd, args, cwd = ROOT) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', code => {
      ...
      reject(new Error(`${cmd} failed (${code}): ${err.slice(-4000)}`));
    });
  });
}
...
function cleanVtt(vtt) {
  ...
  for (const block of blocks) {
    ...
    const text = raw
      .filter(x => !/^\d+$/.test(x) && !/^\d{2}:\d{2}(?::\d{2})?[.,]\d{3}\s+-->/.test(x) && !/^NOTE\b/i.test(x))
      .join(' ')
      .replace(/<[^>]+>/g, '')
...
async function getTranscriptFromCaptions(dir, url) {
  ...
  try {
    await run(YTDLP, [
      '--no-playlist', '--no-warnings', '--write-auto-subs', '--write-subs',
      '--sub-langs', 'all', '--sub-format', 'vtt', '--skip-download',
      '-o', path.join(subDir, 'source'), url
    ]);
  } catch (e) {
    const existing = await fs.readdir(subDir).catch(() => []);
    if (!existing.some(f => f.toLowerCase().endsWith('.vtt'))) throw e;
  }

  const files = (await fs.readdir(subDir)).filter(f => f.toLowerCase().endsWith('.vtt'));
  if (!files.length) {
    throw new Error('No captions were found for this video. Try a YouTube video with subtitles/captions enabled.');
  }
...
async function translate(text, target) {
  ...
  const out = [];
  for (const part of parts) out.push(await translateChunk(part, target));
  return out.join(' ');
}
...
async function tts(text, language, outPath, dir) {
  ...
  for (let i = 0; i < parts.length; i++) {
    const txt = path.join(dir, `tts-${i}.txt`);
    const mp3 = path.join(dir, `tts-${i}.mp3`);
...
async function processJob(job) {
  ...
  try {
    ...
    if (!transcript) {
      throw new Error('For keyless mode, the video must have captions. Use a YouTube video with subtitles enabled.');
    }
    ...
    await tts(translated, job.language, dubbed, dir);

    setJob(job.id, { status: 'rendering video', progress: 88 });
    const output = path.join(dir, 'translated.mp4');
    // Keep the original video stream untouched; only replace its audio stream.
    await run(FFMPEG, [
      '-y', '-i', input, '-i', dubbed,
      '-map', '0:v:0', '-map', '1:a:0',
      ...
      // Fallback without an invalid duration flag; preserve the original video.
      await run(FFMPEG, [
        '-y', '-i', input, '-i', dubbed,
        '-map', '0:v:0', '-map', '1:a:0',
    ...
    if (job.uploadPath) await fs.unlink(job.uploadPath).catch(() => {});
  }
}
...
app.use('/files', express.static(ROOT, { fallthrough: false }));
...
app.post('/api/dub', async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: e?.message || 'Could not start dubbing.' });
  }
});
...
app.post('/api/dub/upload', upload.single('video'), async (req, res) => {
  try {
    ...
    if (!req.file || !langCodes[lang]) {
      if (req.file) await fs.unlink(req.file.path).catch(() => {});
      return res.status(400).json({ error: 'Video and supported target language are required.' });
    }
    const id = crypto.randomUUID();
...
app.get('/api/dub/:id', (req, res) => {
  ...
  const { uploadPath, ...safe } = job;
  return res.json(safe);
});

app.use(express.static(DIST));
app.use((req, res, next) => {
  if (req.method !== 'GET' || req.path.startsWith('/api/') || req.path.startsWith('/files/') || req.path === '/healthz') return next();
...
const server = app.listen(PORT, HOST, () => {
  console.log(`LinguaDub keyless server listening on ${HOST}:${PORT}`);
  console.log(`Writable work directory: ${ROOT}`);
});
server.on('error', err => {
